	...
	while (1) {
		...
		scanf("%d", &korean);
			if (korean < 0 && korean > 101) {
				printf("다시 입력해주세요"); // WARN. 왜 이게 안뜨지?---------------------------
			}
			else if (korean >= 0 && korean <= 100) break;
	}
...
개선점 : 배열 개선, 숫자 점수입력 값 초과시 다시 묻도록 개선, else 다시 입력해주세요 부분을 E학점으로 대체

문제점 : 숫자점수를 잘못 입력했을시 "다시 입력해주세요" 출력이 되지않음, 점수 입력시 숫자가 아닌 문자를 넣으면 해당 점수입력칸이 무한반복되는 오류발생
